...
- passenger env-vars loading under python3.x is enabled via the `SenEnv` entry in the project's httpd/passenger.conf section.
  - usage: `SetEnv EZSCAN__ENV_SETTINGS_PATH /path/to/BUL_CBP__env_settings.sh`
...
For activating the virtual-environment manually, don't source the settings file directly. Instead, add to `project_env/bin/activate`:
  export EZSCAN__ENV_SETTINGS_PATH="/path/to/EZSCAN__env_settings.sh"
  source $EZSCAN__ENV_SETTINGS_PATH
...
ENV_SETTINGS_FILE = os.environ['EZSCAN__ENV_SETTINGS_PATH']  # set in `httpd/passenger.conf`, and `env/bin/activate`

## update path
sys.path.append( PROJECT_DIR_PATH )

## reference django settings
os.environ[u'DJANGO_SETTINGS_MODULE'] = 'easyscan_config.settings'  # so django can access its settings
