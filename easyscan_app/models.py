...
class RequestViewGetHelper( object ):
    ...
    def __init__( self ):
        ...
        self.AVAILABILITY_API_2021_URL_ROOT = os.environ['EZSCAN__2021_AVAILABILITY_API_URL_ROOT']
    ...
    def hit_availability_api( self, bibnum ):
        """ Hits availability-api with bib for title.
            Called by check_title() """
        log.debug( 'bibnum, ``%s``' % bibnum )
        try:
            availability_api_url = '%s/v2/bib_items/%s/' % ( self.AVAILABILITY_API_2021_URL_ROOT, bibnum )
            log.debug( 'availability_api_url, ``%s``' % availability_api_url )
            r = requests.get( availability_api_url )
            d = r.json()
            title = d['response']['bib']['title']
        except Exception as e:
            log.debug( 'RequestViewGetHelper(); exception, %s' % unicode(repr(e)) )
            title = ''
        return title
